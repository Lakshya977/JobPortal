...
const Hero = () => {
  const { searchFilter, setSearchFilter, setIsSearch } = useContext(AppContext); // Adjusted the context variable names
  ...
  // Array of logos
  const companyLogos = [microsoft, adobe, walmart, accenture, samsung, amazon];
    ...
    <>
      <div className="bg-gradient-to-r from-purple-800 to-purple-600 py-12 px-4 flex flex-col items-center min-h-[60vh]">
        <div className="w-full max-w-5xl text-center">
          ...
          <div className={`bg-white rounded-full shadow-xl flex items-center px-4 py-2 w-full max-w-3xl mx-auto transition-all duration-500 ${searchClicked ? 'ring-2 ring-blue-400 scale-105' : ''}`}>
            ...
            <button
              onClick={onSearch}
              className={`ml-4 bg-blue-600 hover:bg-blue-700 text-white px-5 py-2 rounded-full text-sm sm:text-base font-semibold transition-transform duration-300 active:scale-95 ${searchClicked ? 'animate-pulse' : ''}`}
            >
              Search
            </button>
          </div>
